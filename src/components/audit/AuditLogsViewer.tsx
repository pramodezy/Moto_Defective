...
export const AuditLogsViewer: React.FC<AuditLogsViewerProps> = ({ logs }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Timeline List */}
      <div className="rounded-xl border border-[#1c2b53] bg-[#101a35] p-6 shadow-lg">
        <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-3 before:bottom-3 before:w-0.5 before:bg-[#1f2e5a]">
          {filteredLogs.map((log) => (
            <div key={log.id} className="relative group">
              {/* Dot Icon */}
              <div className="absolute -left-6 top-1 flex items-center justify-center w-6 h-6 rounded-full bg-[#0b1329] border border-[#1f2e5a] group-hover:border-cyan-500 transition-colors shadow">
                {getActionIcon(log.action)}
              </div>

              {/* Log Card */}
              <div className="p-4 rounded-xl bg-[#0b1329] border border-[#1f2e5a] hover:border-cyan-500/30 transition-all text-xs">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {log.awb && (
                      <span className="font-mono text-[11px] text-amber-300 px-1.5 py-0.2 rounded bg-amber-500/10 border border-amber-500/20">
                        AWB: {log.awb}
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-2 text-slate-400 flex items-center gap-2">
                  <span>Operator:</span>
                  <strong className="text-slate-200">{log.user_name}</strong>
                ...
                {(log.old_status || log.new_status) && (
                  <div className="mt-2 flex items-center gap-2 text-slate-300">
                    ...
                    <span>→</span>
                    <span className="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-mono border border-indigo-500/30">
                      {log.new_status}
                    </span>
                  </div>
                )}
                ...
                {/* Remarks */}
                {log.remarks && (
                  <p className="mt-2 text-slate-300 bg-[#101a35] p-2.5 rounded-lg border border-[#1c2b53]/80">
                    {log.remarks}
                  </p>
                )}
              </div>
